...
isHLine l = B.length l >= 4 && myAll (`elem` ("=-_"::String)) l
isParaLine l = not (isListLine l) &&
               not (isPreLine l) &&
               not (B.null l) &&
               not (fst (parseHeader l) > 0) &&
               not (isSpecialLine l) &&
               not (isHLine l)
...
parseInline wi t | B.null t              = []
  ...
  where (link, lrest)     = B.span (not . (== ']')) (B.tail t)
        ...
        isNormalNonWord c = not (isAlphaNum c) && not (c `elem` ("[!"::String))
        ...
        isWebLinkChar c   = isAlphaNum c || c `elem` (":/_.-~?=#"::String) -- more to add?
...
isValidPagename = myAll (\c -> isAlphaNum c || c `elem` ("_-/"::String) ) 
